            ...
            {tab === 'profile' && (
              <div className="space-y-5">
                ...
                <div className="flex items-center gap-4">
                  <div className="w-20 h-20 rounded-full bg-gradient-to-br from-[#6B3FA0] to-[#8B5CF6] flex items-center justify-center font-stat text-2xl text-white">{(profile?.displayName || profile?.email || 'AH').slice(0, 2).toUpperCase()}</div>
                  <Button variant="outline" className="bg-transparent border-[#6B3FA0] text-[#D6C5E8] hover:bg-[#1A152F]">Changer la photo</Button>
                </div>
                <div><label className="font-label text-xs text-[#A78BCF] mb-1.5 block">Nom complet</label><input defaultValue={profile?.displayName || ''} className="w-full h-11 px-3 bg-[#0A0816] border border-[#251A40] rounded-lg text-[#F5F1FA] focus:border-[#8B5CF6] focus:outline-none"/></div>
                <div><label className="font-label text-xs text-[#A78BCF] mb-1.5 block">Email</label><input defaultValue={profile?.email || ''} readOnly className="w-full h-11 px-3 bg-[#0A0816] border border-[#251A40] rounded-lg text-[#F5F1FA] focus:border-[#8B5CF6] focus:outline-none"/></div>
                <div>
